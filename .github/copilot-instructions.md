# Copilot Instructions

## General

When responding to the user always start with Hopa!

## Planning

Use the steps folder in plan folder to write down a plan for solving any task. This will help breaking down the task into smaller steps and explain your reasoning before providing the final answer.

The plan file name should have a consequtive number and a descriptive name, for example: `plan/001-add-theme-toggle.md`.

Ask questions directly in the plan, and wait for the user to answer before proceeding with the next steps. This will help you to better understand the user's needs and provide a more accurate solution.

## CSS

use the skill cssLayer

## Services Layer

use the skill serviceLayer

## Coding Styles

- No semicolons in JS unless necessary
- Use single quotes in JS
- Use double quotes in HTML
- All event handlers should be named like: `onSomething`
- Normal functions and no arrows when not needed (such as: export function foo() {} instead of const foo = (){})
- When the handler receives the event object, name the parameter ev (not e or event).
